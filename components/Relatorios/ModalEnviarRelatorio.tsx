  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Enviar por E-mail">
      <form onSubmit={handleEnviar} className="space-y-5">
        <div>
           <label className="block text-[10px] font-black text-faint uppercase mb-1">E-mail do Cliente</label>
           <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="w-full p-4 bg-surface-2 border border-subtle rounded-xl font-bold" />
        </div>
        <div>
           <label className="block text-[10px] font-black text-faint uppercase mb-1">Assunto</label>
           <input type="text" required value={assunto} onChange={e => setAssunto(e.target.value)} className="w-full p-4 bg-surface-2 border border-subtle rounded-xl font-bold" />
        </div>
        <div>
           <label className="block text-[10px] font-black text-faint uppercase mb-1">Mensagem</label>
           <textarea rows={5} value={mensagem} onChange={e => setMensagem(e.target.value)} className="w-full p-4 bg-surface-2 border border-subtle rounded-xl text-sm" />
        </div>
        ...
        <div className="flex gap-3 pt-2">
           <button type="button" onClick={onClose} className="flex-1 py-4 font-black text-muted uppercase text-xs border border-subtle rounded-xl">Cancelar</button>
