  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Configurar Relatório">
      <div className="space-y-6">
        <div>
           <label className="block text-xs font-black text-faint uppercase tracking-widest mb-1">Período de Referência</label>
           <input 
             type="text" 
             value={periodo} 
             onChange={e => setPeriodo(e.target.value)}
             className="w-full p-4 bg-surface-2 border border-subtle rounded-2xl font-bold outline-none" 
             placeholder="Ex: Janeiro 2026"
           />
        </div>

        <div className="space-y-3">
           <h4 className="text-xs font-black text-faint uppercase tracking-widest">Seções a Incluir</h4>
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {SECOES_OPCOES.map(opt => (
                <label key={opt.id} className="flex items-center gap-3 p-3 bg-surface-2 rounded-xl cursor-pointer hover:bg-surface-2 transition-colors">
                   <input 
                    type="checkbox" 
                    checked={secoes.includes(opt.id)} 
                    onChange={() => handleToggleSecao(opt.id)}
                    className="h-5 w-5 rounded-lg text-blue-600 border-subtle"
                   />
                   <span className="text-xs font-bold text-main">{opt.label}</span>
                </label>
              ))}
           </div>
        </div>
