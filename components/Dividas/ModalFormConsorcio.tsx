...
const ModalFormConsorcio: React.FC<Props> = ({ open, onClose, onSave, initialData, clienteId }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 bg-surface-3/40 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
            <div className="bg-surface rounded-[16px] shadow-xl w-full max-w-4xl overflow-hidden animate-slide-up flex flex-col max-h-[90vh]">
                <div className="px-6 py-4 border-b border-subtle flex justify-between items-center bg-surface-2">
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-muted uppercase tracking-wider mt-0.5">Regras isentas de Juros Compostos</p>
                    </div>
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                    <div className="bg-sky-50 text-sky-700 px-4 py-3 rounded-[8px] text-[11px] font-bold tracking-wider uppercase flex gap-3 mb-6 items-start border border-sky-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div className="col-span-full border-b border-subtle pb-2 mb-2">
                        ...
                        <div>
                            ...
                            <select name="asset_type" value={formData.asset_type || 'real_estate'} onChange={handleChange} className="w-full bg-surface-2 border border-subtle text-main text-[12px] font-bold rounded-[8px] px-3 h-9 outline-none">
                                {ASSET_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                            </select>
                        </div>

                        {/* Seção 2: Financeiro Base */}
                        <div className="col-span-full border-b border-subtle pb-2 mb-2 mt-4">
                            <h3 className="text-[10px] font-black tracking-widest text-faint uppercase">2. Estrutura da Carta</h3>
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-muted uppercase tracking-wider mb-1.5">Valor da Carta (R$)</label>
                            <Input type="number" name="credit_letter_value" value={formData.credit_letter_value || ''} onChange={handleChange} min={0} />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-muted uppercase tracking-wider mb-1.5">Parcela Atual (R$)</label>
                        ...
                        <div>
                            ...
                            <select name="contract_type" value={formData.contract_type || 'fixed_installment'} onChange={handleChange} className="w-full bg-surface-2 border border-subtle text-main text-[12px] font-bold rounded-[8px] px-3 h-9 outline-none">
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-muted uppercase tracking-wider mb-1.5">Total Pago até Agora (R$)</label>
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-muted uppercase tracking-wider mb-1.5">Taxa de Adm. (Total %)</label>
                            <Input type="number" name="admin_fee_total" value={formData.admin_fee_total || ''} onChange={handleChange} step="0.01" />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-muted uppercase tracking-wider mb-1.5">Fundo de Reserva (%)</label>
                            <Input type="number" name="reserve_fund_rate" value={formData.reserve_fund_rate || ''} onChange={handleChange} step="0.01" />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-muted uppercase tracking-wider mb-1.5">Seguro de Vida (Opcional - R$ Mensal)</label>
                        ...
                        <div>
                            ...
                            <select name="monetary_index" value={formData.monetary_index || 'INCC'} onChange={handleChange} className="w-full bg-surface-2 border border-subtle text-main text-[12px] font-bold rounded-[8px] px-3 h-9 outline-none">
                                {INDEX_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-muted uppercase tracking-wider mb-1.5">Correção Acumulada (%)</label>
                        ...
                        <div>
                            ...
                            <select name="contemplation_status" value={formData.contemplation_status || 'not_contemplated'} onChange={handleChange} className="w-full bg-surface-2 border border-subtle text-main text-[12px] font-bold rounded-[8px] px-3 h-9 outline-none">
                        ...
                        <div>
                            ...
                            <select name="bid_strategy" value={formData.bid_strategy || 'none'} onChange={handleChange} className="w-full bg-surface-2 border border-subtle text-main text-[12px] font-bold rounded-[8px] px-3 h-9 outline-none">
                        ...
                        <div className="col-span-full flex flex-col md:flex-row gap-6 mt-2 pt-4 bg-surface-2 p-4 rounded-[8px] border border-subtle">
                            <label className={`flex items-center gap-3 cursor-pointer ${formData.asset_type !== 'real_estate' ? 'opacity-50' : ''}`} title="Apenas válido para imóveis (Rule B6)">
                                <input type="checkbox" name="fgts_eligible" checked={formData.fgts_eligible} onChange={handleChange} disabled={formData.asset_type !== 'real_estate'} className="w-5 h-5 rounded border-subtle text-sky-600 focus:ring-sky-500" />
                                <div>
                                    <div className="text-[11px] font-bold text-main uppercase tracking-wider">Elegível ao FGTS</div>
                                    <div className="text-[9px] text-faint">Pode usar fundo de garantia para lances (Regra B6)</div>
                                </div>
                            </label>
                            ...
                            <label className={`flex items-center gap-3 cursor-pointer ${formData.contemplation_status === 'not_contemplated' ? 'opacity-50' : ''}`} title="Necessita estar contemplado (Rule B7)">
                                <input type="checkbox" name="asset_released" checked={formData.asset_released} onChange={handleChange} disabled={formData.contemplation_status === 'not_contemplated'} className="w-5 h-5 rounded border-subtle text-sky-600 focus:ring-sky-500" />
                                <div>
                                    <div className="text-[11px] font-bold text-main uppercase tracking-wider">Carta Liberada (Ativo Comprado)</div>
                                    <div className="text-[9px] text-faint">Recurso do consórcio já foi utilizado (Regra B7)</div>
                                </div>
                            </label>
                        </div>
                    </div>
                ...
                <div className="px-6 py-4 border-t border-subtle flex justify-end gap-3 bg-surface-2">
                    ...
                    <Button variant="primary" onClick={handleSalvar} isLoading={salvando} className="bg-sky-600 hover:bg-sky-700 h-9 px-4 text-[11px] font-bold uppercase tracking-wider">
