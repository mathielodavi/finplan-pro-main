...
import { Home, ChevronDown, ChevronUp, Plus, Trash2, Edit2, X, Check, Loader2 } from 'lucide-react';
...
const TIPOS = ['Residencial', 'Automotivo'];
...
const AcordeoProtecaoPatrimonial: React.FC<Props> = ({ dados, defaultAberto }) => {
    ...
    const salvar = async () => {
        ...
        try {
            ...
            setErroSalvar('Erro ao salvar proteção patrimonial: ' + (err?.message || 'tente novamente.'));
        } finally {
            setSalvando(false);
        }
    };
            ...
            <button onClick={() => setAberto(v => !v)}
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                        ...
                        <p className="text-[14px] font-bold text-main tracking-tight leading-none">Proteção Patrimonial</p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    {extras.length > 0 ? (
                ...
                <div className={`space-y-5 ${defaultAberto ? '' : 'border-t border-subtle px-5 py-5'}`}>
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <button onClick={() => openModal()}
                            ...
                            <Plus size={13} /> Adicionar Proteção Patrimonial
                        </button>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <p className="text-sm text-faint text-center py-4">Nenhuma proteção patrimonial cadastrada.</p>
                    )}
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-surface-3/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-surface rounded-2xl shadow-xl w-full max-w-md p-5 space-y-5">
                        <div className="flex items-center justify-between border-b border-subtle pb-3">
                            <h3 className="text-[16px] font-bold text-main tracking-tight">{form.id ? 'Editar' : 'Nova'} Proteção Patrimonial</h3>
...
export default AcordeoProtecaoPatrimonial;
