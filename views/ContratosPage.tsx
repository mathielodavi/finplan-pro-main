...
const ContratosPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-surface-2 flex flex-col">
      <nav className="bg-surface border-b border-subtle px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-xl font-bold tracking-tight text-main">Vibe <span className="text-blue-600">Financeiro</span></span>
        </div>
        <div className="flex items-center space-x-6">
          <div className="hidden md:flex flex-col text-right">
            <span className="text-sm font-bold text-main">{user?.user_metadata?.full_name || 'Consultor'}</span>
          ...
          <button onClick={() => logout()} className="px-4 py-2 text-sm font-bold text-muted hover:text-main">Sair</button>
        </div>
      </nav>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 sm:p-8 space-y-8">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            <h1 className="text-4xl font-black text-main tracking-tight">Gestão de Contratos</h1>
            <p className="text-muted font-medium">Controle os acordos de planejamento e serviços extras.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <select 
              value={clienteId}
              onChange={(e) => setClienteId(e.target.value)}
              className="px-4 py-3 bg-surface border border-subtle rounded-xl shadow-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all font-bold text-main min-w-[250px]"
            >
        ...
        {clienteId ? (
          loading && !contratos.length ? (
            <div className="py-20 text-center animate-pulse">
               ...
               <p className="text-faint font-bold uppercase tracking-widest text-xs">Carregando Contratos...</p>
            </div>
          ) : (
            <ListaContratos 
              contratos={contratos} 
          ...
          <div className="bg-surface p-20 rounded-3xl border border-subtle text-center space-y-4">
            <div className="h-20 w-20 bg-surface-2 rounded-full inline-flex items-center justify-center text-faint">
               <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
               </svg>
            </div>
            <p className="text-muted font-medium">Selecione um cliente para visualizar e gerenciar seus contratos.</p>
          </div>
        )}
      </main>
      ...
      <footer className="py-6 text-center text-faint text-[10px] font-black uppercase tracking-[0.2em] border-t border-subtle bg-surface">
